...
/*
    ...
    public List<TreatmentResponse> getTreatmentByName(String name, int page, int pageSize) {
        if (page < 0 || pageSize <= 0) {
            throw new IllegalArgumentException("Not found" + page ", pageSize=" + pageSize);
        }

        PageRequest pageReq = PageRequest.of(page, pageSize);
        Page<Treatment> treatment = repository.findByName(pageReq);
        return treatment.getContent().stream()
                .map(TreatmentMapper::toResponse)
                .toList();
    }
*/
